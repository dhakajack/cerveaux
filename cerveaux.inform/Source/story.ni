...
After going south from Couloir 1 when the consciousness of the player is 4 for the first time:
	now the BlockChatterFlag is true;
	say "Vous voyez votre reflet dans le miroir et vous le regardez un longue moment. Vous reconnaissez immédiatement Julien, vêtu de son uniforme de garde bleue et contre toute attente portant toujours son chapeau officiel.";
	try looking.
...
"Si le zombisme s'est propagé si vite, s'inquiète Lucky, penses-tu que ma famille est survécu?[line break]-- Oui, je le sais, reassure la scientifique. Les Drummonds travaille ici, dans l'installation et tous les travailleurs sont hébergés dans l'enceinte. Tu n'es pas loin d'eux. [line break]-- Génial! dit le chien.", 
...
"Quand j'ai vu le corps de Julien entrer dans le sas, j'ai remarqué qu'il déplaçait avec difficulté, marchant lentement et trébuchant, dit la scientifique.[line break]-- Oui, je sais comment marcher, dit la tranche de cerveau, mais je le trouve désespérément difficile de le mettre en pratique.[line break]-- La souris n'aide pas, commente Lucky. Il bouge toujours les jambes.[line break]-- Moi? dit le souris avec indignation. C'est toi qui veut toujours aller à quatre pattes.[line break]-- OK, dit la scientifique, puis-je vous convaincre qu'une suele personne doit pilote le corps à la fois?[line break]-- Oui, certainement, acquiesce la tranche de cerveau. Le volant est à toi.", 
"D'accord, dit la scientifique. Je m'empresse de faire ce qu'il faut pour obtenir le remède.[line break]-- Bon. Assez de papotage mes amis, laissez-la concentrer, conseil la tranche de cerveau.[line break]-- Oui, d'accord, disent le chien et la souris."
}
...
"Si j'ai bien compris, dit Lucky, tu as dit que les cerveaux des zombies sont complètement effacés par le virus. Vu que rien ne reste de son cerveau, comment se fait-il qu'il peut fonctionner maintenant?[line break]-- Nous n'avons jamais eu un pareil cas pour étudier, répond la scientifique. Il est temps de réviser notre compréhension du processus: il semble que les cerveaux ne sont pas détruits, mais plutôt supprimés. Ses voies neuronales ont toujours étés intactes. En fait, c'est grâce à eux que nous continuons à exister du tout. Nous nous servons de ses voies pour formuler nos propres pensées.",
...
"Alors, que faire? demande la souris.[line break]-- Julian, dit Isabelle, il faut parler avec le colonel pour le convaincre que tu es guéri et que nous possédons un remède contre le virus.[line break]-- D'accord. Je ferai mon mieux, dit Julien.[line break]-- Allons-y! crie la souris."
}.
